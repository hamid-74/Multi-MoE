...
model4 = AutoModelForSeq2SeqLM.from_pretrained(local_path_3).to(device)
...
def calculate_expert_distance(models):
    expert_keys = [
    ...
    for block_num in range(1, 13, 2):  # Assuming 12 blocks in the encoder
        for layer_num in range(1, 2):  # Assuming each block has 1 layer
            for expert_num in range(num_experts):
                for pair in pairs:
                    model1_state_dict = models[pair[0]].state_dict()
                    model2_state_dict = models[pair[1]].state_dict()
...
encoder_distances, decoder_distances = calculate_expert_distance(models)
...
def merge_models(models, sorted_keys, sorted_values):
    ...
    for i in range(len(sorted_keys)):
        ...
        if model_num == 1:
            ...
            merged_model_state_dict[wi_key] = model4_state_dict[wi_key]
            merged_model_state_dict[wo_key] = model4_state_dict[wo_key]

    
    return merged_model_state_dict

# Merge the experts
merged_model_base_state_dict = merge_models(models, sorted_keys, sorted_values)
...
merged_model.save_pretrained("merged-emre-base-sst2-mnli")

print("Merged model saved in 'merged-emre-base-sst2-mnli' folder.")
