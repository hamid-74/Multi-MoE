...
class ExecStats:
    ...
    def calculate_time_to_first_token(self):
        # Start with model_change
        time_to_first_token = self.model_change

        # Add prefill embedding times
        time_to_first_token += sum(self.embed['prefill'])

        # Add prefill layer times
        for layer_times in self.layer['prefill'].values():
            # Add attention times
            time_to_first_token += sum(layer_times['attention'])

            # Add expert times
            time_to_first_token += sum(layer_times['expert']['time'])

        # Add lm_head prefill times
        time_to_first_token += sum(self.lm_head['prefill'])
...
class MultiMoE:
    def __init__(self, args, model_ids, model_layout, tokenizer):
        ...
        print(f"total available memory: {self.total_available_memory}")
        ...
        # with open("expert_map.json", "w") as f:
        #     json.dump(self.expert_map.tolist(), f)
    ...
    def measure_model_dist(self):
        ...
        for i_layer in range(self.n_layer):
            for i_expert in range(self.n_expert):
                ...
                temp_expert_1 = self.flatten_expert_weights(self.models[self.model_ids[0%len(self.model_ids)]].model.layers[i_layer].block_sparse_moe.experts[i_expert])
                temp_expert_2 = self.flatten_expert_weights(self.models[self.model_ids[1%len(self.model_ids)]].model.layers[i_layer].block_sparse_moe.experts[i_expert])
                l2_norm = torch.norm( temp_expert_1 - temp_expert_2, p=2).item()
                # print(f"l2_norm:{l2_norm}")
                self.expert_dist[i_layer][i_expert] = l2_norm


        json_string = json.dumps(self.expert_dist.tolist())

        # Save to a JSON file
        with open('/multi-llm/multi-moe/Multi-MoE/expert_dist.json', 'w') as f:
            f.write(json_string)
    ...
    def swap_main_model(self, new_model_id):
        self.lm_head.to('cpu')
        self.model.embed_tokens.to('cpu')
        self.model.norm.to('cpu')

        for i in range(len(self.model.layers)):
            self.model.layers[i].self_attn.to('cpu')
            self.model.layers[i].input_layernorm.to('cpu')
            self.model.layers[i].block_sparse_moe.gate.to('cpu')
            self.model.layers[i].post_attention_layernorm.to('cpu')
            # only model.layers[i].block_sparse_moe.experts is on CPU
        
        self.main_model_id = new_model_id
        self.model_layout["non_expert"] = new_model_id
        self.model = self.models[self.main_model_id].model
        self.lm_head = self.models[self.main_model_id].lm_head
    ...
    def generate(self, text, output_token=1, input_token=None, input_ids_flag=False, print_flag=False, temperature=1, do_sample=False, ppl_flag=False, record_stats=False, new_model_id=None
    ):
        ...
        # swap non-expert params.
        if(new_model_id):
            if(new_model_id != self.main_model_id):
                s_time = time.time()
                self.swap_main_model(new_model_id)
                e_time = time.time()
                exec_stats.model_change = e_time - s_time
        ...
        is_decode = False
        for i_token in range(output_token+1):
            # tick = time.time()
            if print_flag:
            ...
            # input_ids = outputs  # Use the entire generated output
    ...
    def mixtral_forward(self, input_ids, position_ids, is_decode, exec_stats, record_stats=False, is_causal=True):
        ...
        # print(f"seq length:{self.past_key_values_length}")

        if(is_decode):
            attention_mask = torch.ones((1, self.past_key_values_length + 1), device='cuda:0')
        else:
            attention_mask = None
        ...
        for i_layer, layer in enumerate(self.model.layers):
            # print(f"forward inps:{inps}")
            # print(f"forward inps shape:{inps.shape[1]}")
            s_time = time.time()
